...
1. Run the test suite and capture output: `./gradlew test` for unit tests, `./gradlew connectedAndroidTest` for instrumented.
...
3. For each failing test, determine the root cause category:
   ...
   - **Environment issue**: Missing config, emulator issue, timing problem.
...
5. Apply the fix:
   ...
   - Add synchronization or `IdlingResource` for flaky UI tests.
